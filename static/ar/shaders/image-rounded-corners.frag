...
void main() {
	...
	if(vUV.x / aspectRatio <= radius && vUV.y <= radius) {
	...
	} else if(vUV.x / aspectRatio <= radius && vUV.y >= 1.0 - radius) {
		if(sqrt(pow(radius - vUV.x / aspectRatio, 2.0) + pow(1.0 - radius - vUV.y, 2.0)) <= radius) {
			gl_FragColor = vec4(texture2D(src, vec2(vUV.x, vUV.y)).xyz, opacity);
		} else {
			gl_FragColor = vec4(0, 0, 0, 0);
		}
