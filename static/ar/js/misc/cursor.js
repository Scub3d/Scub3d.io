$( document ).ready(function() {
	...
	function checkIfMarkerHasLoaded() {
		let marker = document.querySelector("[cursor]");
		console.log(marker.hasLoaded);

	    if(marker.hasLoaded === false) {
	       window.setTimeout(checkIfMarkerHasLoaded, 100);
	    } else {
	      fixCursorRaycast();
	    }
	}

	checkIfMarkerHasLoaded();

	function fixCursorRaycast() {
		/* * * * * * * * * * * * * * * * * * * * * *
			...
			return function(evt) {
				...
				* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
				direction
					...
					.applyMatrix4(matrix.copy(camera.projectionMatrix).invert())
...
// put stack overflow credit here